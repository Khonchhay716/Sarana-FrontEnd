...
export default function FormProduct({ onClose, onSubmit, productBack, formlayout, Title, inputClass, dailogSize }: Props) {
  ...
  const renderField = (field: SingleField) => {
    ...
    const inputStyle = `w-full p-2 border rounded text-orange-500 ${isError ? "border-red-500" : "border-amber-50"}`;
      ...
      <div key={key} className={`${colClass} ${className || ""}`}>
        ...
        {type === "select" ? (
          ...
          >
            ...
            {allOptions.map((o) => (
              <option key={JSON.stringify(o.value)} value={JSON.stringify(o.value)}>{o.label}</option>
            ))}
          </select>
        ) : type === "file" ? (
          <>
            <input id={key} type="file" accept={accept} className="hidden" onChange={(e) => handleImageUpload(key, e.target.files?.[0] as File)} />
            <label htmlFor={key} className={`block p-2 rounded text-center cursor-pointer border ${isError ? "border-red-500" : "border-amber-50"}`}>{imagePreviews[key] ? "Change Image" : placeholder || "Choose a file"}</label>
            {imagePreviews[key] && <img src={imagePreviews[key]} alt="preview" className="mt-2 h-28 object-cover rounded border" />}
          </>
        ) : type === "checkbox" ? (
          <input type="checkbox" checked={!!formData[key]} onChange={(e) => handleChange(key, e.target.checked)} className="h-5 w-5" />
        ) : type === "radio" && options ? (
          <div className="flex gap-4">{options.map((o) => (
            <label key={o.value} className="flex items-center gap-1">
              <input type="radio" name={key} checked={formData[key] === o.value} onChange={() => handleChange(key, o.value)} />{o.label}
            </label>
          ))}</div>
        ) : type === "textarea" ? (
          <textarea className={inputStyle} value={formData[key] ?? ""} onChange={(e) => handleChange(key, e.target.value)} rows={4} />
        ) : (
          <input type={type} className={inputStyle} value={formData[key] ?? ""} onChange={(e) => handleChange(key, type === "number" ? Number(e.target.value) || 0 : e.target.value)} placeholder={placeholder} />
        )}
