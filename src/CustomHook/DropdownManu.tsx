    ...
    <div className="relative inline-block text-left z-50" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className={`absolute z-10 mt-2 w-50 text-black border-1 border-black rounded-2xl overflow-hidden max-h-[300px] overflow-y-auto scrollbar-hide bg-white ${inputClass}`} >
          ...
          {options.map((option) => (
            <button key={option.option} onClick={() => { onSelect(option.option); setIsOpen(false); }} className="w-full text-left px-4 py-2 bg-white-200 hover:bg-gray-300 cursor-pointer mt-0.5 flex">
