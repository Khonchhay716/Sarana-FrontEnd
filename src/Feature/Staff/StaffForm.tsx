...
const StaffForm = ({ staffId, onClose }: StaffFormProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (staffId) {
            await updateData("Staff", staffId, payload as any, () => setTimeout(() => handleClose(), 500), undefined,
                async () => {
                    if (hasNewFile && imageUrl) await deleteImage(imageUrl);
                }
            );
        } else {
            await createData("Staff", payload as any, () => setTimeout(() => handleClose(), 500), false,
                async () => {
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
                        ...
                        <div className={`px-4 sm:px-6 py-3 border-t flex-shrink-0 ${dl ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-white"}`}>
                            <div className="flex justify-end gap-2 sm:gap-3">
                                ...
                                <ComponentPermission scopes={[staffId ? "staff:update" : "staff:create"]}>
                                    <button type="submit" disabled={loading || uploadingImage}
                                        ...
                                        {loading || uploadingImage ? (
                                            ...
                                            <>
                                                <span className="hidden sm:inline">{staffId ? "Update Staff" : "Create Staff"}</span>
                                                <span className="sm:hidden">{staffId ? "Update" : "Create"}</span>
                                            </>
                                        )}
                                    </button>
                                </ComponentPermission>
