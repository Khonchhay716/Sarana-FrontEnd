...
import ResetPasswordForm from "../Customer/Resetpasswordform";
...
interface StaffPersonFormProps {
    staffId: number;
    staffName: string;
...
const StaffPersonForm = ({ staffId, staffName, userId, onClose }: StaffPersonFormProps) => {
    ...
    const [formData, setFormData] = useState<StaffPersonFormData>({
        username: "",
        email: "",
        password: "",
        confirmPassword: "",
            ...
            if (data.roles && Array.isArray(data.roles)) {
                setSelectedRoles(
                    data.roles.map((r: any) => ({
                        id: r.id,
                        name: r.name || r.roleName || String(r.id),
                        value: r.id,
                        data: r,
                    }))
                );
    ...
    const handleRoleChange = (values: MultiValue) => {
        ...
        setFormData(prev => ({
            ...prev,
            roleIds: values.map(v => Number(v.id)),
        }));
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!isEditMode) {
            if (!formData.password) { alertError("Password is required!"); return; }
            if (formData.password.length < 6) { alertError("Password must be at least 6 characters!"); return; }
            if (formData.password !== formData.confirmPassword) { alertError("Passwords do not match!"); return; }
        }
    ...
    return (
        <>
                    ...
                    <div className={`px-6 py-4 border-b flex-shrink-0 ${dl ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
                        <div className="flex justify-between items-start">
                            <div className="flex items-start gap-3">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${dl ? "bg-indigo-900/30" : "bg-indigo-100"}`}>
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${dl ? "bg-indigo-900/40 text-indigo-300" : "bg-indigo-100 text-indigo-700"}`}>
                                            {staffName}
                                        </span>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
                        ...
                        >
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                {isEditMode && userId && (
                                    <div className={`rounded-xl border-2 p-4 ${dl ? "border-orange-700/40 bg-orange-900/10" : "border-orange-200 bg-orange-50"}`}>
                                        <div className="flex items-center justify-between gap-3">
                                            <div>
                                                <p className={`text-sm font-bold ${dl ? "text-orange-300" : "text-orange-700"}`}>🔑 Password</p>
                                            ...
                                            <button
                                                ...
                                                className="flex-shrink-0 px-4 py-2 rounded-lg text-xs font-semibold bg-orange-500 hover:bg-orange-600 text-white transition-colors shadow"
                                            >
                                                Reset Password
                                            </button>
                                        </div>
                            ...
                            <div className="flex justify-end gap-3">
                                ...
                                <ComponentPermission scopes={[isEditMode ? "user:update" : "user:create"]}>
                                    <button
                                        type="submit"
                                        disabled={loading}
                                        className={`px-8 py-1 rounded-lg font-medium transition-all shadow-lg ${loading ? "bg-indigo-400 cursor-not-allowed" : "bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700"
            ...
            {/* Reset Password — renders on top (z-[70]) */}
...
export default StaffPersonForm;