...
const StockOutForm = ({ initialProduct, onClose, onSuccess }: StockOutFormProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (isSerialProduct) {
            ...
            if (productBase.stock !== undefined && effectiveQuantity > productBase.stock) {
                alertError(`Cannot remove ${effectiveQuantity}. Only ${productBase.stock} in stock!`); return;
            }
        }
        ...
        try {
            ...
            alertify.success("Remove stock success");
    ...
    const addModeTabClass = (active: boolean) => active
        ? (dl ? "bg-red-900/40 border-red-600 text-red-300" : "bg-red-50 border-red-400 text-red-700")
                        ...
                        <div className="min-w-0 flex-1">
                            {productBase ? (
                                ...
                                <div>
                                    ...
                                    <p className={`text-xs sm:text-sm mt-0.5 ${dl ? "text-gray-400" : "text-gray-500"}`}>Remove stock from inventory</p>
                                </div>
                            )}
                        </div>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
                        <div className="overflow-y-auto flex-1 px-4 sm:px-6 py-5 custom-scrollbar"
                            ...
                            <div className="space-y-5">
                                {/* Product (no supplier for stock out) */}
                                {isStandalone && (
                                            ...
                                            <div>
                                                <label className={labelClass}>
                                                    ...
                                                    {serialNumbers.length > 0 && (
                                                        <span className={`ml-2 px-2 py-0.5 rounded-full text-xs font-bold ${dl ? "bg-red-900/30 text-red-300" : "bg-red-100 text-red-700"}`}>
                                                ...
                                                {serialAddMode === "csv" && (
                                                    <div className="space-y-3">
                                                        <div onClick={() => !showColPicker && csvInputRef.current?.click()}
                                                            className={`flex flex-col items-center justify-center gap-2 p-8 rounded-xl border-2 border-dashed transition-all ${showColPicker ? "opacity-40 cursor-not-allowed" : "cursor-pointer"} ${dl ? "border-gray-600 hover:border-red-500 hover:bg-red-900/10" : "border-gray-300 hover:border-red-400 hover:bg-red-50/50"}`}>
                                                                ...
                                                                <div className="fixed inset-0 flex items-center justify-center z-[70] p-4 pointer-events-none">
                                                                    <div className={`rounded-2xl shadow-2xl w-full max-w-md pointer-events-auto overflow-hidden ${dl ? "bg-gray-800 border border-gray-700" : "bg-white"}`} onClick={e => e.stopPropagation()}>
                                                                        <div className={`px-5 py-4 border-b flex items-center gap-3 ${dl ? "border-gray-700 bg-gray-800/80" : "border-gray-200 bg-gray-50"}`}>
                                                                            <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${dl ? "bg-red-900/40" : "bg-red-100"}`}>
                                                                                <svg width={18} height={18} viewBox="0 0 24 24" fill="none" stroke={dl ? "#fca5a5" : "#dc2626"} strokeWidth={2} strokeLinecap="round">
                                                                        ...
                                                                        <div className={`px-5 py-3 border-b flex items-center gap-3 ${dl ? "border-gray-700" : "border-gray-200"}`}>
                                                                            <input type="checkbox" checked={csvHasHeader} onChange={e => setCsvHasHeader(e.target.checked)} className="w-4 h-4 rounded cursor-pointer accent-red-500" />
                                                                            <div>
                                                                        ...
                                                                        <div className={`px-5 py-3 ${dl ? "" : "bg-gray-50/50"}`}>
                                                                            <p className={`text-sm font-semibold ${dl ? "text-red-300" : "text-red-700"}`}>Which column is the Serial Number?</p>
                                                                        </div>
                                                                            ...
                                                                            {csvHeaders.map((header, i) => {
                                                                                ...
                                                                                return (
                                                                                    <button key={i} type="button" onClick={() => setCsvColIndex(i)}
                                                                                        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border text-left transition-all ${isSelected ? (dl ? "border-red-500 bg-red-900/30 ring-1 ring-red-500/30" : "border-red-500 bg-red-50 ring-1 ring-red-200") : (dl ? "border-gray-600 hover:border-gray-500 bg-gray-700/30" : "border-gray-200 hover:border-gray-300 bg-white")}`}>
                                                                                        <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all ${isSelected ? "border-red-500" : (dl ? "border-gray-500" : "border-gray-400")}`}>
                                                                                            {isSelected && <div className="w-2 h-2 rounded-full bg-red-500" />}
                                                                                        </div>
                                                                                        <div className="min-w-0 flex-1">
                                                                                            <p className={`text-sm font-semibold truncate ${isSelected ? (dl ? "text-red-300" : "text-red-700") : (dl ? "text-gray-200" : "text-gray-700")}`}>{header}</p>
                                                                                            {samples && <p className={`text-xs font-mono truncate mt-0.5 ${dl ? "text-gray-500" : "text-gray-400"}`}>e.g. {samples}</p>}
                                                                                        </div>
                                                                                        <span className={`text-[10px] px-1.5 py-0.5 rounded font-mono flex-shrink-0 ${isSelected ? (dl ? "bg-red-700 text-red-200" : "bg-red-100 text-red-700") : (dl ? "bg-gray-700 text-gray-400" : "bg-gray-100 text-gray-500")}`}>Col {i + 1}</span>
                                                                                    </button>
                                                                                );
                                                                            })}
                                                                        </div>
                                                                        <div className={`px-5 py-4 border-t flex gap-2 ${dl ? "border-gray-700 bg-gray-800/50" : "border-gray-200 bg-gray-50"}`}>
                                                                            <button type="button" onClick={() => applyCSVImport(csvRows, csvColIndex)} className="flex-1 py-2.5 bg-red-500 hover:bg-red-600 text-white text-sm font-bold rounded-xl transition-colors">✓ Import "{csvHeaders[csvColIndex]}"</button>
                                                                            <button type="button" onClick={closeColPicker}
                                                    ...
                                                    <div className={`rounded-xl border p-4 min-h-[60px] flex flex-wrap items-center gap-x-1.5 gap-y-2 w-full ${dl ? "border-gray-600" : "border-gray-200"}`}>
                                                        {serialNumbers.length > 0 ? (
                                                            serialNumbers.map((sn) => (
                                                                <span key={sn} className={`bs-item inline-flex items-center gap-1.5 pl-2.5 pr-1 py-1 rounded-full ${dl ? "bg-red-900/30" : "bg-red-50"}`}>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" value={reference} onChange={e => setReference(e.target.value)} className={inputClass} placeholder="e.g. Order #, Return #" />
                                    </div>
                                    <div>
                                        ...
                                        <input type="text" value={note} onChange={e => setNote(e.target.value)} className={inputClass} placeholder="e.g. Damaged, Sold" />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className={`px-4 sm:px-6 py-3 border-t flex-shrink-0 flex flex-col-reverse sm:flex-row justify-end gap-3 ${dl ? "bg-gray-800/80 border-gray-700" : "bg-gray-50 border-gray-200"}`}>
                            ...
                            <ComponentPermission scopes={["stockmovement:create"]}>
                                <button
                                    type="submit"
                                    disabled={saving || (!productBase || effectiveQuantity <= 0)}
                                    className={`flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl font-bold text-sm text-white transition-all shadow-md ${saving ? "opacity-70 cursor-not-allowed bg-red-500" : "bg-red-600 hover:bg-red-700 active:scale-95"} disabled:active:scale-100 disabled:opacity-50 disabled:cursor-not-allowed`}
                                >
                                    {saving ? (
                                        <>
                                            <Spinner size={4} />
