...
const InfoItem = ({ label, value, icon, darkLight }: { label: string; value: string; icon?: string; darkLight: boolean }) => (
    <div>
        <p className={`text-xs font-semibold mb-1 ${darkLight ? "text-gray-500" : "text-gray-400"}`}>{label}</p>
        <p className={`text-sm font-medium flex items-center gap-1.5 ${darkLight ? "text-gray-200" : "text-gray-800"}`}>
            {icon && <span>{icon}</span>}
            {value || "N/A"}
        </p>
    </div>
...
const StockAdjustmentDetail = ({ id, onClose }: { id: number; onClose: () => void }) => {
    ...
    useEffect(() => {
        ...
        const fetchDetail = async () => {
            try {
                const res = await AxiosApi.get(`stock/adjustments/${id}`, {
                    signal: abortController.signal
                });
                setData(res?.data?.data ?? res?.data);
            } catch (err: any) {
                if (!abortController.signal.aborted) {
                    alertError("Failed to load adjustment details.");
                    onClose();
                }
            } finally {
                setLoading(false);
    ...
    const typeInfo = TYPE_LABEL[data?.typeAdjustment];
            ...
            <div className={`fixed top-[65px] left-0 right-0 bottom-0 z-40 flex items-center justify-center py-8 sm:py-1 pointer-events-none transition-all duration-300 ${isAnimating ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}>
                <div className={`rounded-2xl shadow-2xl w-full max-w-lg max-h-full flex flex-col overflow-hidden pointer-events-auto transform transition-all duration-300 ${darkLight ? "bg-gray-800" : "bg-white"} ${isAnimating ? "translate-y-0" : "translate-y-4"}`}>
                    ...
                    <div className={`px-6 py-4 border-b flex justify-between items-center flex-shrink-0 ${darkLight ? "border-gray-700" : "border-gray-200"}`}>
                        <h2 className={`text-lg font-bold ${darkLight ? "text-white" : "text-gray-900"}`}>Adjustment Detail</h2>
                        ...
                        {loading ? (
                            ...
                            <div className="space-y-6">
                                <div className={`flex items-center gap-3 pb-4 border-b border-dashed ${darkLight ? 'border-gray-600' : 'border-gray-200'}`}>
                                    ...
                                    <div>
                                        <p className={`font-bold text-base ${darkLight ? "text-white" : "text-gray-900"}`}>{data.productName}</p>
                                ...
                                <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                                    <InfoItem label="Type" value={typeInfo?.label ?? "N/A"} icon={typeInfo?.icon} darkLight={darkLight} />
                                    <InfoItem label="Adjusted Qty" value={String(data.qualityAdjustment)} darkLight={darkLight} />
                                    <InfoItem label="Reason" value={data.reason} darkLight={darkLight} />
                                    <InfoItem label="Cost Value" value={`$ ${data.costPrice?.toFixed(2) ?? '0.00'}`} darkLight={darkLight} />
                                    ...
                                    <div className={`col-span-2 pt-3 mt-2 border-t ${darkLight ? "border-gray-700" : "border-gray-100"}`}>
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className={`text-2xl ${typeInfo?.label === "Over" ? "text-emerald-500" : "text-red-500"}`}>
                                                {typeInfo?.label === "Over" ? "→" : "←"}
                                            </div>
                                            <div className="flex-1 text-center p-2 rounded-lg bg-gray-500/10">
                                ...
                                {data.note && (
                                    <div className={`p-3 rounded-xl ${darkLight ? "bg-gray-700/40" : "bg-gray-50"}`}>
                                        ...
                                        <p className={`text-sm ${darkLight ? "text-gray-200" : "text-gray-800"}`}>{data.note}</p>
                                    </div>
                                )}
