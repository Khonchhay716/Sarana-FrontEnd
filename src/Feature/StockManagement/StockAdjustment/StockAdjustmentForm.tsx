...
// Matches POS.Domain.Enums.TypeAdjustment
enum TypeAdjustment {
    Over = 0,
    Lost = 1,
}

// Matches POS.Domain.Enums.AdjustmentReason
...
const REASON_OPTIONS = [
    { value: AdjustmentReason.Damaged, label: "Damaged" },
    { value: AdjustmentReason.Lost, label: "Lost" },
    { value: AdjustmentReason.Correction, label: "Correction" },
    { value: AdjustmentReason.Other, label: "Other" },
];

const StockAdjustmentForm = ({ initialProduct, onClose, onSuccess }: StockAdjustmentFormProps) => {
    ...
    const [typeAdjustment, setTypeAdjustment] = useState<TypeAdjustment>(TypeAdjustment.Over);
    const [qualityAdjustment, setQualityAdjustment] = useState<string>("");
    const [reason, setReason] = useState<number>(AdjustmentReason.Damaged);
    ...
    const closeColPicker = () => {
        setShowColPicker(false); setCsvHeaders([]); setCsvRows([]); setCsvColIndex(0); setCsvHasHeader(true); setBulkError("");
    };

    const resetEntryState = () => {
        setSerialNumbers([]); setQualityAdjustment(""); setNote(""); setReason(AdjustmentReason.Damaged);
        setTypeAdjustment(TypeAdjustment.Over);
    ...
    const effectiveQuantity = isSerialProduct ? serialNumbers.length : (qualityAdjustment === "" ? 0 : Number(qualityAdjustment));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (isSerialProduct) {
            ...
            if (effectiveQuantity <= 0) { alertError("Quantity adjustment must be greater than 0!"); return; }
        }

        const payload = {
            productId: productBase.id,
            typeAdjustment: typeAdjustment,
            qualityAdjustment: effectiveQuantity,
            reason: reason,
            note: note || "",
        ...
        try {
            ...
            alertError(err?.response?.data?.message || "Failed to save stock adjustment.");
        } finally {
            setSaving(false);
        }
    };
    ...
    const selectedType = ADJUSTMENT_TYPES.find(t => t.value === typeAdjustment)!;
                    ...
                    <div className={`px-4 sm:px-6 py-3 border-b flex-shrink-0 flex justify-between items-center gap-3 ${dl ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
                        <div className="min-w-0 flex-1">
                            {productBase ? (
                                ...
                                <div>
                                    <h2 className={`text-xl font-bold truncate ${dl ? "text-white" : "text-gray-900"}`}>⚖️ Stock Adjustment</h2>
                                    <p className={`text-xs sm:text-sm mt-0.5 ${dl ? "text-gray-400" : "text-gray-500"}`}>Correct inventory discrepancies</p>
                                </div>
                            )}
                        </div>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
                        <div className="overflow-y-auto flex-1 px-4 sm:px-6 py-5 custom-scrollbar"
                            ...
                            <div className="space-y-5">
                                ...
                                {/* Adjustment Type - shown immediately, no need to pick a product first */}
                                {!loadingProduct && (
                                    <div>
                                        <label className={labelClass}>Adjustment Type <span className="text-red-500">*</span></label>
                                        <div className="grid grid-cols-2 gap-3">
                                            {ADJUSTMENT_TYPES.map(t => {
                                                const isActive = typeAdjustment === t.value;
                                                return (
                                                    <button
                                                        key={t.value}
                                                        type="button"
                                                        onClick={() => setTypeAdjustment(t.value)}
                                                        className={`flex items-center justify-center gap-2 py-2 px-4 rounded-xl border-2 transition-all font-semibold text-sm ${isActive ? t.color[dl ? "dark" : "light"] : t.inactive[dl ? "dark" : "light"]}`}
                                                    >
                                ...
                                {/* Quantity / Serial Numbers - still needs the product, since it depends on productType */}
                                {productBase && !loadingProduct && (
                                    <>
                                        {isSerialProduct ? (
                                                    ...
                                                    <div className={`rounded-xl border p-4 min-h-[60px] flex flex-wrap items-center gap-x-1.5 gap-y-2 w-full ${dl ? "border-gray-600" : "border-gray-200"}`}>
                                                        {serialNumbers.length > 0 ? (
                                                            serialNumbers.map((sn) => (
                                                                <span key={sn} className={`bs-item inline-flex items-center gap-1.5 pl-2.5 pr-1 py-1 rounded-full ${typeAdjustment === TypeAdjustment.Over ? (dl ? "bg-emerald-900/30" : "bg-emerald-50") : (dl ? "bg-red-900/30" : "bg-red-50")}`}>
                                                                    <span className={`text-sm font-mono ${typeAdjustment === TypeAdjustment.Over ? (dl ? "text-emerald-200" : "text-emerald-700") : (dl ? "text-red-200" : "text-red-700")}`}>{sn}</span>
                                            ...
                                            <div>
                                                <label className={labelClass}>Quantity Adjustment <span className="text-red-500">*</span></label>
                                                <input
                                                    type="number"
                                                    min={0}
                                                    value={qualityAdjustment}
                                                    onChange={e => setQualityAdjustment(e.target.value === "" ? "" : String(Math.max(0, Number(e.target.value))))}
                                ...
                                {/* Reason & Note - shown immediately, no need to pick a product first, but stay positioned under Quantity / Serial Numbers */}
                                {!loadingProduct && (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" value={note} onChange={e => setNote(e.target.value)} className={inputClass} placeholder="e.g. Found during audit" />
                                        </div>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className={`px-4 sm:px-6 py-3 border-t flex-shrink-0 flex flex-col-reverse sm:flex-row justify-end gap-3 ${dl ? "bg-gray-800/80 border-gray-700" : "bg-gray-50 border-gray-200"}`}>
                            ...
                            <ComponentPermission scopes={["adjustment:create"]}>
                                <button
                                    ...
                                    className={`flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl font-bold text-sm text-white transition-all shadow-md ${saving ? "opacity-70 cursor-not-allowed" : ""} ${typeAdjustment === TypeAdjustment.Over ? (saving ? "bg-emerald-500" : "bg-emerald-600 hover:bg-emerald-700 active:scale-95") : (saving ? "bg-red-500" : "bg-red-600 hover:bg-red-700 active:scale-95")} disabled:active:scale-100 disabled:opacity-50 disabled:cursor-not-allowed`}
                                >
                                    {saving ? (
                                        <>
                                            <Spinner size={4} />
                                        ...
                                        <>
                                            ...
                                            Save {selectedType.label} Adjustment
                                        </>
                                    )}
                                </button>
                            </ComponentPermission>
