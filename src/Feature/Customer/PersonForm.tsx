...
const PersonForm = ({ customerId, customerName, userId, onClose }: PersonFormProps) => {
    ...
    // 👇 NEW: store selected roles as MultiValue objects so XSelectSearch can display them
    ...
    const loadUserData = async () => {
        ...
        if (data) {
            ...
            // 👇 Pre-populate the XSelectSearch with existing roles
    ...
    // 👇 NEW: handle role selection changes
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // roleIds is already kept in sync via handleRoleChange
        if (isEditMode && userId) {
            const payload = {
                ...
                roleIds: formData.roleIds,   // ✅ now populated from XSelectSearch
                staffId: null,
                customerId: customerId,
            };
                    ...
                    <div className={`px-6 py-4 border-b flex-shrink-0 ${dl ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
                        <div className="flex justify-between items-start">
                            <div className="flex items-start gap-3">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${dl ? "bg-teal-900/30" : "bg-teal-100"}`}>
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${dl ? "bg-teal-900/40 text-teal-300" : "bg-teal-100 text-teal-700"}`}>
                                            {customerName}
                                        </span>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
                        ...
                        >
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                {/*Role Select — shown in BOTH create and edit mode */}
                                <div>
