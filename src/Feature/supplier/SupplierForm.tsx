...
const SupplierForm = ({ supplierId, onClose }: SupplierFormProps) => {
    const { darkLight } = useGlobleContextDarklight();
    const { createData, updateData, GetDatabyID, loading } = HookIntergrateAPI<SupplierFormData>();
    ...
    const loadSupplierData = async () => {
        ...
        const data: any = await GetDatabyID("Suppliers", supplierId);
        if (data) {
            setFormData({
                id: data.id,
                name: data.name || "",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.name.trim()) { alertError("Supplier name is required!"); return; }
        ...
        if (supplierId) {
            await updateData("Suppliers", supplierId, payload as any, () => setTimeout(() => handleClose(), 500));
        } else {
            await createData("Suppliers", payload as any, () => setTimeout(() => handleClose(), 500), false);
        }
    };
                        ...
                        <div className="flex justify-between items-start gap-3">
                            <div className="min-w-0">
                                ...
                                <p className={`text-xs sm:text-sm mt-0.5 truncate ${dl ? "text-gray-400" : "text-gray-500"}`}>
                                    {supplierId ? "Update supplier information" : "Fill in the details to create a new supplier"}
                                </p>
                            </div>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
                            ...
                            <div className="flex flex-col gap-5">
                                ...
                                <div>
                                    ...
                                    <textarea name="address" value={formData.address} onChange={handleInputChange}
                                        className={`${inputClass} resize-none`} placeholder="Enter supplier address" rows={3} />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className={`px-4 sm:px-6 py-3 border-t flex-shrink-0 ${dl ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-white"}`}>
                            <div className="flex justify-end gap-2 sm:gap-3">
                                ...
                                <ComponentPermission scopes={[supplierId ? "supplier:update" : "supplier:create"]}>
                                    <button type="submit" disabled={loading}
                                        ...
                                        {loading ? (
                                            ...
                                            <>
                                                <span className="hidden sm:inline">{supplierId ? "Update Supplier" : "Create Supplier"}</span>
                                                <span className="sm:hidden">{supplierId ? "Update" : "Create"}</span>
                                            </>
                                        )}
                                    </button>
                                </ComponentPermission>
