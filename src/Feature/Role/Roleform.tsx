...
const RoleForm = ({ roleId, onClose }: RoleFormProps) => {
    const { darkLight } = useGlobleContextDarklight();
    const { createData, updateData, GetDatabyID, loading } = HookIntergrateAPI<RoleFormData>();
    ...
    const loadRoleData = async () => {
        if (!roleId) return;
        const data: any = await GetDatabyID("Roles", roleId);
        if (data) {
            setFormData({
                id: data.id,
                name: data.name || "",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.name.trim()) {
            alertError("Role name is required!");
            return;
        }
        if (!formData.description.trim()) {
            alertError("Description is required!");
            return;
        }

        if (roleId) {
            await updateData("Roles", roleId, formData, () => {
                setTimeout(() => {
                    handleClose();
                }, 500);
            });
        } else {
            await createData("Roles", formData, () => {
                setTimeout(() => {
                    handleClose();
                }, 500);
            });
    ...
    return (
        <>
            ...
            {/* Modal */}
            <div
                className={`fixed inset-0 flex items-center justify-center z-50 p-4 pointer-events-none transition-all duration-300 ${isAnimating ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
                    }`}
            >
                <div
                ...
                >
                    {/* Header */}
                    <div className={`px-4 sm:px-6 py-3 sm:py-4 border-b flex-shrink-0 ${darkLight ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
                        <div className="flex justify-between items-start gap-3">
                            <div className="min-w-0">
                                <h2 className={`text-base sm:text-2xl font-bold truncate ${darkLight ? "text-white" : "text-gray-900"}`}>
                                    {roleId ? "Edit Role" : "Add New Role"}
                                </h2>
                                <p className={`text-xs sm:text-sm mt-0.5 truncate ${darkLight ? "text-gray-400" : "text-gray-500"}`}>
                                    {roleId ? "Update role information" : "Fill in the details to create a new role"}
                                </p>
                            </div>
                            <button onClick={handleClose}
                                className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-xl transition-all
                ${darkLight ? "text-gray-400 hover:text-gray-200 hover:bg-gray-700" : "text-gray-500 hover:text-gray-700 hover:bg-gray-100"}`}>
                                ×
                            </button>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
                        ...
                        {/* Footer */}
                        <div className={`px-4 sm:px-6 py-3 border-t flex-shrink-0 ${darkLight ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-white"}`}>
