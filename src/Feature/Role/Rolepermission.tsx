...
const Permissions: FC = () => {
  ...
  return (
    <div className={`w-full min-h-screen ${dl ? "bg-gray-900" : "bg-gray-50"}`} style={{marginTop:'-20px'}}>
      <div className="p-3 sm:p-6">

        {/* ===== STICKY PAGE HEADER ===== */}
        <div
          className={`sticky top-[-17px] z-30 -mx-3 sm:-mx-6 mt-0 px-3 sm:px-6 py-3 sm:py-4 mb-4 sm:mb-6 backdrop-blur-md border-b 
            ${dl ? "bg-gray-900/80 border-gray-700" : "bg-gray-50/80 border-gray-200"}`}
            style={{height:'85px', marginTop:'-20px'}}
        >
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              <MdAdminPanelSettings
                className={`w-7 h-7 sm:w-9 sm:h-9 drop-shadow-lg animate-bounce flex-shrink-0
                  ${dl ? "text-purple-400" : "text-purple-600"}`}
              />
              <div className="min-w-0">
                <h4
                  className={`font-bold text-base sm:text-2xl truncate
                    ${dl ? "text-white" : "text-gray-900"}`}
                >
                  PERMISSION MANAGEMENT
                </h4>
                {roleData && (
        ...
        <div className={`rounded-xl shadow overflow-y-auto ${dl ? "bg-gray-800" : "bg-white"}`}>
            ...
            <div className="flex flex-col items-center justify-center py-20">
              <div
                className={`animate-spin rounded-full h-12 w-12 border-b-2
                  ${dl ? "border-purple-400" : "border-purple-500"}`}
              />
              <p className={`mt-4 text-sm ${dl ? "text-gray-400" : "text-gray-500"}`}>
                Loading permissions...
              </p>
          ...
          {/* Permission Groups */}
          {!loading &&
            roleData?.allPermissions.map((group, groupIndex) => (
              <div
                key={group.group}
                className={`border-b last:border-b-0 p-3 sm:p-5
      ...
      {/* ===== FOOTER ===== */}
      <div
        className={`flex flex-col sm:flex-row items-center justify-between border-t px-4 py-3 gap-1
          ${dl ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-white"}`}
      >
        <p className={`text-xs ${dl ? "text-gray-500" : "text-gray-400"}`}>
          Sarana Project 2025-2026
        ...
        <p className={`text-xs ${dl ? "text-gray-500" : "text-gray-400"}`}>
          ...
          <Link
            ...
            className={`font-medium
              ${dl ? "text-purple-400 hover:text-purple-300" : "text-purple-600 hover:text-purple-700"}`}
          >
            STUDENT AT NORTON UNIVERSITY
          </Link>
        </p>
