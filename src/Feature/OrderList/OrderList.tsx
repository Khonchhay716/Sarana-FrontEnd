...
// import { ShoppingCart, ChevronDown, X, Calendar, Filter, TrendingUp, Eye, Package, Tag, CreditCard, User, Clock, Hash, Wrench, Shield } from 'lucide-react';
...
//     { value: 1, label: 'Unpaid' }, { value: 2, label: 'Paid' }, { value: 3, label: 'Partial' },
// ];
// const TYPE_PRODUCT = [
...
//                         className={`px-4 py-2.5 text-sm cursor-pointer text-gray-400 ${darkLight ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}>All</div>
//                     {options.map(opt => (
//                         <div key={opt.value} onClick={() => { onChange(opt.value); setOpen(false); }}
//                             className={`px-4 py-2.5 text-sm cursor-pointer font-medium
//                                 ${value === opt.value ? (darkLight ? 'bg-indigo-900 text-indigo-300' : 'bg-indigo-50 text-indigo-600') : (darkLight ? 'hover:bg-gray-700 text-gray-200' : 'hover:bg-gray-50 text-gray-700')}`}>
//                             {opt.label}
//                         </div>
//                     ))}
//                 </div>
...
//                     ${(fromDate || toDate) ? 'bg-indigo-500 border-indigo-500 text-white' : darkLight ? 'bg-gray-700 border-gray-600 text-gray-200 hover:bg-gray-600' : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-50'}`}>
...
//                                     ${activePreset?.label === p.label ? 'bg-indigo-500 text-white' : darkLight ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}`}>
//                                 {p.label}
//                             </button>
//                         ))}
...
//                             { lbl: 'To', val: toDate, set: (v: string) => onChange(fromDate, v) }].map(({ lbl, val, set }) => (
//                                 <div key={lbl}>
...
//             <div className={`relative w-full max-w-3xl max-h-[87vh] overflow-y-auto custom-scrollbar rounded-xl shadow-2xl ${darkLight ? 'bg-gray-900 border border-gray-700' : 'bg-white border border-gray-100'}`}
//                 style={{ paddingRight: '6px', scrollbarWidth: 'thin', scrollbarColor: '#9ca3af transparent' }}
//             >
//                 {/* Header */}
//                 <div className={`sticky top-0 z-10 flex items-center justify-between px-6 py-2 border-b ${darkLight ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-100'}`}>
//                     <div className="flex items-center gap-3">
//                         <div className={`p-2 rounded-xl ${darkLight ? 'bg-indigo-900' : 'bg-indigo-50'}`}>
//                             <ShoppingCart className={`w-5 h-5 ${darkLight ? 'text-indigo-400' : 'text-indigo-600'}`} />
//                         </div>
//                         <div>
//                             <h2 className={`font-bold text-lg font-mono ${darkLight ? 'text-indigo-300' : 'text-indigo-600'}`}>{order.orderNumber}</h2>
...
//                         </div>
//                     </div>
//                     <button onClick={onClose} className={`p-2 rounded-xl transition-colors ${darkLight ? 'hover:bg-gray-700 text-gray-400' : 'hover:bg-gray-100 text-gray-500'}`}>
//                         <X className="w-5 h-5" />
//                     </button>
//                 </div>
...
//                         </span>
//                         <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold ${darkLight ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-600'}`}>
//                             {order.saleType?.name}
//                         </span>
//                     </div>
...
//                         ].map(info => (
//                             <div key={info.label} className={`flex items-center gap-3 p-3 rounded-xl ${darkLight ? 'bg-gray-800' : 'bg-gray-50'}`}>
//                                 <div className={`p-1.5 rounded-lg shadow-sm ${darkLight ? 'bg-gray-700 text-gray-300' : 'bg-white text-gray-500'}`}>{info.icon}</div>
//                                 <div>
//                                     <p className="text-xs text-gray-400">{info.label}</p>
//                                     <p className={`text-sm font-semibold ${darkLight ? 'text-gray-200' : 'text-gray-700'}`}>{info.value}</p>
//                                 </div>
//                             </div>
//                         ))}
//                     </div>
...
//                     {/* Order Items */}
//                     <div>
//                         <h3 className={`text-sm font-bold mb-3 flex items-center gap-2 ${darkLight ? 'text-gray-200' : 'text-gray-700'}`}>
...
//                                 return (
//                                     <div key={item.id} className={`rounded-xl border overflow-hidden ${darkLight ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-100'}`}>
//                                         {/* Item row */}
...
//                                                 : <div className={`w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 text-xl font-bold ${darkLight ? 'bg-gray-700 text-gray-400' : 'bg-gray-200 text-gray-500'}`}>
//                                                     {item.productName?.charAt(0)}
//                                                 </div>
//                                             }
//                                             <div className="flex-1 min-w-0">
//                                                 <div className="flex items-center gap-2 flex-wrap">
//                                                     <p className={`font-semibold text-sm truncate ${darkLight ? 'text-white' : 'text-gray-800'}`}>{item.productName}</p>
//                                                     {/* Serial badge */}
//                                                     {isSerial && (
//                                                         <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold ${darkLight ? 'bg-violet-900/60 text-violet-300' : 'bg-violet-100 text-violet-700'}`}>
//                                                             S/N
//                                                         </span>
//                                                     )}
//                                                 </div>
...
//                                                         <span className={`font-mono ${darkLight ? 'text-violet-400' : 'text-violet-600'}`}>
//                                                             {item.serialNo.name}
//                                                         </span>
//                                                     )}
//                                                 </div>
...
//                                                 <p className={`font-bold text-sm ${darkLight ? 'text-white' : 'text-gray-800'}`}>${item.subTotal.toFixed(2)}</p>
//                                             </div>
//                                         </div>

//                                         {/* Warranty bar — only for serial items */}
//                                         {isSerial && item.warrantyMonths && item.warrantyMonths > 0 && (
//                                             <div className={`flex items-center gap-3 px-3 py-2 border-t text-xs ${darkLight ? 'border-gray-700 bg-violet-900/20' : 'border-violet-100 bg-violet-50'}`}>
//                                                 <Shield className={`w-3.5 h-3.5 shrink-0 ${darkLight ? 'text-violet-400' : 'text-violet-500'}`} />
//                                                 <span className={`font-semibold ${darkLight ? 'text-violet-300' : 'text-violet-700'}`}>
...
//                                                 </span>
//                                                 <span className={`font-bold ${darkLight ? 'text-gray-500' : 'text-gray-400'}`}>→</span>
//                                                 <span className={`font-semibold ${darkLight ? 'text-violet-300' : 'text-violet-700'}`}>
...
//                     {/* Summary */}
//                     <div className={`rounded-xl p-4 space-y-2 ${darkLight ? 'bg-gray-800' : 'bg-gray-50'}`}>
//                         <h3 className={`text-sm font-bold mb-3 ${darkLight ? 'text-gray-200' : 'text-gray-700'}`}>Summary</h3>
...
//                             <span>Total</span>
//                             <span className={darkLight ? 'text-indigo-300' : 'text-indigo-600'}>${order.totalAmount.toFixed(2)}</span>
//                         </div>
//                     </div>
//                 </div>
//             </div>
...
//         {
//             title: 'Order', key: 'order', width: 210,
//             render: (_, record) => (
//                 <div>
...
//             render: (_, record) => (
//                 <div>
//                     <p className={`text-xs mt-0.5 flex items-center gap-1 ${darkLight ? 'text-gray-700' : 'text-gray-700'}`}>
//                         <Clock className="w-3 h-3" />
...
//         },
//         {
//             title: 'Staff', key: 'staff', width: '150px', align: 'center',
//             render: (_, record) => (
//                 <div className="inline-flex items-center">
//                     <span className={`text-sm font-medium ${darkLight ? 'text-gray-200' : 'text-gray-700'}`}>{record.staff?.name ?? 'N/A'}</span>
//                 </div>
//             ),
//         },
//         {
//             title: 'Items', key: 'items', align: 'center', width: '120px',
//             render: (_, record) => (
...
//                                 ${darkLight ? 'bg-gray-600 text-gray-300' : 'bg-gray-100 text-gray-500'}`}>
//                                 {item.productName?.charAt(0)}
//                             </div>
//                     ))}
//                     {record.orderItems?.length > 1 && (
//                         <span className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold
//                             ${darkLight ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-500'}`}>
//                             +{record.orderItems.length - 1}
//                         </span>
//                     )}
//                 </div>
...
//             render: (_, record) => (
//                 <div className="inline-flex items-center">
//                     <p className={`font-bold text-sm ${darkLight ? 'text-white' : 'text-gray-800'}`}>${record?.cashReceived?.toFixed(2)}</p>
//                 </div>
//             ),
//         },
//         {
...
//             render: (_, record) => (
//                 <div className="inline-flex items-center">
//                     <p className={`font-bold text-sm ${darkLight ? 'text-white' : 'text-gray-800'}`}>{record?.pointsUsed}</p>
//                 </div>
//             ),
//         },
//         {
...
//             render: (_, record) => (
//                 <div className="inline-flex items-center">
//                     <p className={`font-bold text-sm ${darkLight ? 'text-white' : 'text-gray-800'}`}>{record.earnedPoints}</p>
//                 </div>
//             ),
//         },
//         {
...
//                     <span className="w-1.5 h-1.5 rounded-full bg-current opacity-70" />{record.status?.name}
//                 </span>
//             ),
//         },
//         {
//             title: 'Pay-Status', key: 'payment', align: 'center',
//             render: (_, record) => (
...
//         },
//         {
//             title: 'Pay-Type', key: 'payment', align: 'center',
//             render: (_, record) => (
//                 <div className="flex items-center gap-0.5">
//                     <span className={`text-xs ${darkLight ? 'text-gray-400' : 'text-gray-400'}`}>{record.paymentMethod?.name}</span>
//                 </div>
//             ),
//         },
//         {
...
//                     ${darkLight ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-600'}`}>
...
//                     ${darkLight ? 'bg-gray-700 text-gray-300' : 'bg-indigo-50 text-indigo-600'}`}>
...
//             {selectedOrder && (
//                 <OrderDetailModal order={selectedOrder} darkLight={darkLight} onClose={() => setSelectedOrder(null)} />
//             )}
//         </>
//     );
// };
