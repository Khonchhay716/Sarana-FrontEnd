...
export default function Admin() {
    ...
    useEffect(() => {
        if (data) {
            const filtered = data.filter((item: { roleId: number }) => item.roleId === 1);
            setDatas(filtered);
        }
    }, [data]);
                            ...
                            <table className={`min-w-full divide-y ${darkLight ? "divide-gray-700" : "divide-gray-200"}`}>
                                <thead className={darkLight ? "bg-gray-800 text-gray-300" : "bg-gray-100 text-gray-700"}>
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 text-left">RoleId</th>
            ...
            {showModal ? <Form
                ...
                formlayout={
                    [
                        ...
                        { key: "profilePicture", label: "Image", type: "file", accept: "image/*", required: true, col: 12 },
                        // {
                        //   key: "profilePicture",
                        //   label: "Profile Image",
                        //   type: "file",
