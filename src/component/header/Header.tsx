...
import StockInSummaryButton from "../../Feature/StockManagement/StockMovement/StockInSummaryButton";
import SalesSummaryModal from "../../Feature/OrderList/showFilterSalesuumary";
...
const Header = () => {
  ...
  const [showFilterSalesuumary, setShowFilterSaleSummary] = useState(false);
  ...
  return (
    <nav className={`navbar px-4 w-full h-[65px] shadow-lg sticky top-0 z-50 flex justify-between items-center border-b-2 transition-all duration-300 ${darkLight
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <button
        onClick={() => setShowFilterSaleSummary(true)}
          ...
          <AiOutlineBarChart className="w-4 h-4" /> Detail Summary
        </button>
      ...
      {showFilterSalesuumary && (
        <SalesSummaryModal
          darkLight={darkLight}
          onClose={() => setShowFilterSaleSummary(false)}
        />
      )}
    </nav>
  );
