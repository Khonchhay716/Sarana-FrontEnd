...
const CoffeeMenuTable: React.FC = () => {
  ...
  const { data } = useFetchDataApi("https://localhost:7095/api/TypeOfProducts");
  ...
  const handleEdit = async (id: number) => { const productData: any = await GetDatabyID("https://localhost:7095/api/TypeOfProducts", id); if (productData) { setEditItem(productData); setShowModal(true); setIsAdd(false); } };
  const handleDelete = (id: number) => { DeleteData("https://localhost:7095/api/TypeOfProducts", id); };
  const handleAddItem = (data: any) => { if (isAdd) { createData("https://localhost:7095/api/TypeOfProducts", data, () => { setShowModal(false); alertify.success("Item added successfully!"); }); } else { updateData("https://localhost:7095/api/TypeOfProducts", data.id, data, () => { setShowModal(false); setEditItem(null); alertify.success("Item updated successfully!"); }); setIsAdd(true); } };
  ...
  return (
    <>
      <div className={`w-full transition-all duration-300 pb-13 ${darkLight ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full table-auto border-collapse shadow-lg rounded-2xl overflow-hidden">
            ...
            <tbody>
              {data.map((item: { id: number; name: string; description: string }, index) => (
      ...
      {showModal ? <Form2 onClose={() => { setShowModal(false); setIsAdd(true); setEditItem(null); }} onSubmit={handleAddItem} productBack={editItem ?? undefined} Title="Form Type of Product " formlayout={[{ row: [{ key: "name", label: "Name", type: "text", required: true, col: 6 }, { key: "description", label: "Description", type: "string", col: 6, required: false }] }]} /> : null}
    </>
  );
};
